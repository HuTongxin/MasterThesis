...
class RPN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_objects=None, dontcare_areas=None):
        ...
        # rpn_cls_score.size() [1,50,W,H]
        rpn_cls_score_reshape = self.reshape_layer(rpn_cls_score, 2)
        # rpn_cls_score_reshape.size() [1,2,50*W,H]
        rpn_cls_prob = F.softmax(rpn_cls_score_reshape)
        # rpn_cls_prob.size() [1,2,50*W,H]
        rpn_cls_prob_reshape = self.reshape_layer(rpn_cls_prob, self.anchor_num*2)
        # rpn_cls_prob_reshape.size() [1,50,W,H]
        # rpn boxes
        rpn_bbox_pred = self.bbox_conv(rpn_conv1)
        # print 'rpn_bbox_pred.std()', rpn_bbox_pred.data.std() * 4
        # rpn_bbox_pred.size() [1,100,W,H]
