...
class Hierarchical_Descriptive_Model(HDN_base):
    def __init__(self, nhidden, n_object_cats, n_predicate_cats, n_vocab, voc_sign,
        ...
        super(Hierarchical_Descriptive_Model, self).__init__(nhidden, n_object_cats, n_predicate_cats, n_vocab,
                                                             voc_sign,
                                                             object_loss_weight, predicate_loss_weight, dropout,
                                                             use_kmeans_anchors, disable_spatial_model, spatial_type)
        ...
        self.fc6_r = FC(512 * 7 * 7, 6000, relu=True)
        self.fc7_r = FC(6000, 2000, relu=True)
        self.fc8_r = FC(33088, nhidden, relu=False)
    ...
    def forward(self, im_data, im_info, gt_objects=None, gt_relationships=None):
        ...
        features, object_rois, rpn_scores = self.rpn(im_data, im_info, gt_objects)

        proposals, after_nms_scores, _ = nms_detections(object_rois[:, 1:].data.cpu().numpy(),
                                                        rpn_scores.data.cpu().numpy(), 0.30)
        ...
        if use_attention_block:
            similarity_s = torch.matmul(resize_s_features, resize_s_features.transpose(0, 1))  # [batch,batch]
            attention_s = F.softmax(similarity_s)
            resize_s_features = torch.matmul(attention_s, resize_s_features)

            similarity_o = torch.matmul(resize_o_features, resize_o_features.transpose(0, 1))  # [batch,batch]
            attention_o = F.softmax(similarity_o)
            resize_o_features = torch.matmul(attention_o, resize_o_features)
        ...
        fc6_r_features = self.fc6_r(resize_r_features)   # 6000
        if self.dropout:
            fc6_r_features = F.dropout(fc6_r_features, training=self.training)
        fc7_r_features = self.fc7_r(fc6_r_features)   # 2000
        if self.dropout:
            fc7_r_features = F.dropout(fc7_r_features, training=self.training)
        cat_r_features = torch.cat((resize_r_features,fc6_r_features,fc7_r_features),1)  # 33088
        r_features = self.fc8_r(cat_r_features)  # 512
