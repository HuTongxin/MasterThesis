...
def anchor_target_layer(rpn_cls_score, gt_boxes, dontcare_areas, im_info, anchor_scales, 
    ...
    # eg. gt_boxes(7,4)  overlaps.shape(W*H*A, 7)
    argmax_overlaps = overlaps.argmax(axis=1)
    # number: W*H*A, which contains value from 0 to 6
    max_overlaps = overlaps[np.arange(len(inds_inside)), argmax_overlaps]
    # number: W*H*A
    gt_argmax_overlaps = overlaps.argmax(axis=0)
    # number: 7, which contains value from 0 to W*H*A-1
    gt_max_overlaps = overlaps[gt_argmax_overlaps,
                               np.arange(overlaps.shape[1])]
    # number: 7
    gt_argmax_overlaps = np.where(overlaps == gt_max_overlaps)[0]
    # it contains value from 0 to W*H*A-1
    ...
    if not cfg.TRAIN.RPN_CLOBBER_POSITIVES:
        ...
        if is_region:
            ...
            labels[max_overlaps < cfg.TRAIN.RPN_NEGATIVE_OVERLAP] = 0   # <0.3: labels = 0

    # fg label: for each gt, anchor with highest overlap
    labels[gt_argmax_overlaps] = 1  # for each gt, anchor with highest overlap: labels = 1
    ...
    if is_region:
        ...
        labels[max_overlaps >= cfg.TRAIN.RPN_POSITIVE_OVERLAP] = 1   # >=0.7: labels = 1
        if cfg.TRAIN.RPN_CLOBBER_POSITIVES:
            # assign bg labels last so that negative labels can clobber positives
    ...
    num_fg = int(cfg.TRAIN.RPN_FG_FRACTION * cfg.TRAIN.RPN_BATCHSIZE)   # num_fg = 0.5 * 512 = 256
    ...
    num_bg = cfg.TRAIN.RPN_BATCHSIZE - num_fg  # 256
    ...
    if DEBUG:
        ...
        # assert np.max(max_overlaps) > cfg.TRAIN.RPN_POSITIVE_OVERLAP
